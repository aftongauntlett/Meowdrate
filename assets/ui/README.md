...
- `UI Bundle.zip` → `PastelUi.png` — toffeecraft's "Pastel Dream" UI kit
  (buttons, bordered panels, icon-buttons in many palette themes; we use
  the blue-toned pieces).
- `RetroCatsPaid.zip` (repo root) → `Cats/AllCats*.png` — another
  toffeecraft cat sprite pack, six palette variants of the same sheet of
  poses. The same pack's `CatItems/Decorations/CatRoomDecorations.png`
  sheet also has a grid of pet bowls (food and water, four color
  variants each) — that's where `water_bowl.png` is cropped from.

All four sources above are toffeecraft (toffeecraft.itch.io) — see
Settings → Credits, which credits the studio once for all of it rather
than listing every individual pack.
